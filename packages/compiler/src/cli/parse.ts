...
const args = yargs(hideBin(process.argv))
  ...
  .example(
    'parse -t simple-expression my-src.json',
    'parse my-src.json into a test sat called simple-expression.json'
  )
  .example(
    'parse -e "const x=10"',
    'parse the expression "const x=10" and write the results to stdout'
  )
...
const result = args.s ? simpleParse(expression) : parse(expression);
...
if (output.path) {
  ...
  fs.writeFileSync(output.path, output.result, { flag: 'w+' });
} else {
