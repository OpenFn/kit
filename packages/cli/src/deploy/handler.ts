...
async function deployHandler(
  ...
  try {
    ...
    if (!process.env.PREFER_LEGACY_DEPLOY && (await fileExists(v2ConfigPath))) {
      ...
      logger.always(
        'Detected openfn.yaml file - switching to v2 deploy (openfn project deploy). Set PREFER_LEGACY_DEPLOY to disable this.'
      );
      return beta.handler(
        {
          ...options,
