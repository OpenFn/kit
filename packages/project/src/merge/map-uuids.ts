...
export default (a: Project, b: Project) => {
  const mapping = new Map<string, MappingRule>();

  const mainProject = a,
    newProject = b;
  for (const newWorkflow of newProject.workflows) {
    const mainWorkflow = mainProject.getWorkflow(newWorkflow.id);
    if (!mainWorkflow) {
      // mainProject.workflows.push(newWorkflow);
      continue;
    }
    ...
    for (const nstep of newWorkflow.steps) {
      ...
      let possibleNodes = getNodesWithParent(nparentsMap.get(nstep.id));
      let found = false;
