...
// This is a real issue found against lightning
test('Should be consistent with name casing', async (t) => {
  const yaml = `id: sandbox-test
name: sandbox-test
cli:
  version: 2
description: abc
...
openfn:
  ...
  inserted_at: 2025-12-19T15:24:22Z
  updated_at: 2026-02-09T15:32:58Z
options:
  ...
  - name: X'Y
    steps:
      - id: a
        ...
        openfn:
          uuid: e2c857d2-5405-4e7e-8187-293730151172
      - id: webhook
        type: webhook
        enabled: true
        openfn:
          uuid: 0e77c576-6c70-49e0-82e3-3473653ccdaa
        next:
          a:
            disabled: false
            condition: always
            openfn:
              uuid: 2c4b0b25-1567-4873-8518-658a22dcbcdd
    history:
      - app:dc5618fdc911
    openfn:
      uuid: 43198acb-41f0-48a0-8d6f-00506fee5b95
      inserted_at: 2026-02-09T16:12:03Z
      updated_at: 2026-02-09T16:12:18Z
      lock_version: 4
    id: x-y
    start: webhook
`;

  const expectedVersion = 'cli:dc5618fdc911';

  const project = await Project.from('project', yaml, { format: 'yaml' });
  const hash = project.workflows[0].getVersionHash();
  t.is(hash, expectedVersion);
});
