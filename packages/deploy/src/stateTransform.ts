...
function stringifyJobBody(body: SpecJobBody): string {
  ...
  function getStateJobCredential(
    specJobCredential: undefined | null | string,
    stateCredentials: ProjectState['project_credentials']
  ): undefined | null | string {
    if (
      specJobCredential &&
      typeof stateCredentials[specJobCredential] === 'undefined'
    ) {
    ...
    return specJobCredential && stateCredentials[specJobCredential].id;
  }
        ...
        if (specJob && !stateJob) {
          return [
            jobKey,
            {
              ...
              project_credential_id: getStateJobCredential(
                specJob.credential,
                credentials
              ),
            },
      ...
      idKeyPairs(project.workflows, state.workflows).map(
        ([key, nextWorkflow, _state]) => {
          ...
          const triggers = Object.fromEntries(
            idKeyPairs(
              nextWorkflow.triggers,
              state.workflows[key].triggers
            ).map(([key, nextTrigger, _state]) => [key, nextTrigger])
          );
