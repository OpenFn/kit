...
test('toNextState sets cron_cursor_job_id when specified', (t) => {
  ...
  t.is(result.workflows.w.triggers.t.cron_cursor_job, jobId);
});

test('toNextState omits cron_cursor_job when not specified', (t) => {
  const state = { workflows: {} };
  const spec = {
    name: 'my project',
    workflows: {
  ...
  const result = mergeSpecIntoState(state, spec);
  t.false('cron_cursor_job' in result.workflows.w.triggers.t);
});
